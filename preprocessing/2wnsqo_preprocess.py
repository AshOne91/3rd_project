...
def find_final_subfolders(root_path: Path):
    ...
    for dirpath, dirnames, _ in os.walk(root_path):
        if not dirnames:  # 더 이상 하위 폴더가 없음
...
def preprocess_text(text: str, stopwords=STOPWORDS) -> str:
    # 1) 한글, 숫자,영어 띄어쓰기만 남기고 나머지 모두 제거
...
if __name__ == "__main__":
    base_path = "unzip_data/초거대 AI 헬스케어 질의응답 데이터/TS"
    categories = ["귀코목질환", "기타", "성형미용 및 재건", "치과질환", "피부질환", "중앙혈액질환"]
    output_file = "QA_random_pair_part2.txt"

    create_txt_from_all_subfolders(base_path, categories, sample_size=3, output_file=output_file)
    input_file = "QA_random_pair_part2.txt"
    output_file = "QA_random_pair_part2_preprocessed.txt"
